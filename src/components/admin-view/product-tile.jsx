  ...
  return (
    <Card className="w-full max-w-sm mx-auto">
      <div>
        ...
        <CardContent>
          <h2 className="text-xl h-[70px] tracking-tighter font-bold mb-2 mt-2">{product?.title}</h2>
          <div className="flex justify-between h-[40px] items-center mb-2">
            <span
              className={`Rs. {
            ...
            {product?.salePrice > 0 ? (
              <span className="text-lg p-1 mb-1  font-bold">Rs. {product?.salePrice}</span>
            ) : null}
          </div>
        </CardContent>
        <CardFooter className="flex justify-between h-[40px] items-center">
          <Button 
            onClick={() => {
